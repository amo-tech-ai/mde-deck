...
export const ValuePropCard: React.FC<ValuePropCardProps> = ({ title, description, icon, color }) => {
    const colorClasses = {
        'deep-blue': 'bg-[#00334F] text-white',
        'off-white': 'bg-white/60 border border-gray-200 text-[#1E1E1E]',
        'mustard': 'bg-[#F3B93C] text-[#1E1E1E]',
    };

    return (
        <motion.div
            className={`p-6 rounded-3xl ${colorClasses[color]}`}
        ...
        >
            ...
            <button className={`mt-4 text-sm font-semibold ${color === 'deep-blue' ? 'text-[#F3B93C]' : 'text-[#E97A41]'}`}>
                Learn More
            </button>
        </motion.div>
    );
